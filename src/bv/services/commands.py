...
from bv.packaging.builder import derive_entry_points_json
...
def build_package(config_path: Path, output: Optional[Path], dry_run: bool) -> Path:
    ...
    with zipfile.ZipFile(target, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        # entry-points.json is a DERIVED artifact from bvproject.yaml.
        # Generated by the single canonical function; never hand-built.
        entry_points_data = derive_entry_points_json(cfg.entrypoints)
...
def run_project(config_path: Path, entrypoint_name: Optional[str] = None, folder_name: Optional[str] = None) -> object:
    ...
    runtime_context.set_runtime_context(
        ...
        folder_id=folder_name,  # Folder name passed via --folder
    )
